...
// 定义一个接口名为list的接口
Mock.mock(baseURL + '/permission', {
    status: 200,
    msg: '登录成功！',
    data: [
        // {id: 5, index: 5, name: "首页", path: '/index', children: []},
        {id: 5, index: 5, name: "学生管理", path: '/student', children: []},
        {id: 1,  index: 1,name: "系统管理", path: '/sys', children: [
            {id: 2, index: 2, name: "人员管理", path: '/sys/user', children: []},
            {id: 3,  index: 3,name: "角色管理", path: '/sys/roles', children: []},
...
Mock.mock(/\/sys\/user/, (options) => {
// Mock.mock(/\/sys/user/g, (options) => {
    let respDataSize = 10;
    let total = 54;
    var po = options.url.indexOf("?"); //获取问号的位置
    if (po != -1) {
        ...
        let pageNo = search['pageNo']
        let maxPage = Math.trunc(total / respDataSize)
        console.log("req", pageNo, maxPage, respDataSize)
        if (pageNo > maxPage) {
            respDataSize = Math.trunc(total % respDataSize)
        }
    } else {
        console.log("no po", po)
    }
    let template = {
        id: '@id', //随机id
        name: '@cname', //随机名称 @name英文名称 @cname中文名称
